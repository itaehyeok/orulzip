...
PERFORMANCE_MEASUREMENT_ATTEMPTS="${PERFORMANCE_MEASUREMENT_ATTEMPTS:-1}"
ASKPASS_FILE=""
...
price_band_schema_migration_enabled() {
  case "$DEPLOY_PRICE_BAND_SCHEMA_MIGRATION" in
    1|true|TRUE|yes|YES|on|ON) return 0 ;;
    auto|AUTO) [ -n "$(price_band_schema_database_url)" ] && return 0 || return 1 ;;
    *) return 1 ;;
  esac
}
...
price_band_schema_database_url() {
  local explicit_url
  explicit_url="$(read_deploy_env_value DEPLOY_SCHEMA_DATABASE_URL || true)"
...
run_price_band_schema_migration() {
  ...
  schema_database_url="$(price_band_schema_database_url)"
  log "running price band cache schema migration on service $DEPLOY_PERFORMANCE_SERVICE"
  if [ -n "$schema_database_url" ]; then
    docker compose -p "$COMPOSE_PROJECT_NAME" run --rm --no-deps -T \
      -e ORULZIP_DB_INIT=0 \
      -e ORULZIP_ENVIRONMENT="$DEPLOY_ENVIRONMENT" \
      -e DATABASE_URL="$schema_database_url" \
      "$DEPLOY_PERFORMANCE_SERVICE" \
