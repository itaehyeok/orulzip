...
- Treat DB schema changes as code. Add schema changes under `migrations/*.sql`, commit them with the application code that needs them, and run `npm run migrate:db` before code that depends on the schema is deployed.
- Whenever a task changes DB schema, DB-backed cache tables, indexes, constraints, seed/reference data, or any query contract that depends on them, explicitly decide whether a migration is required. If it is required, do not consider the task complete until the migration exists, is committed, and the deployment path that runs it has been verified.
- If a DB change is made manually in development or production while debugging, immediately capture the same change as a migration before finishing the task. Manual DB fixes must not remain undocumented one-off state.
- Do not rely on `src/services/db.js` initialization to update production schema. Production deploys run with `ORULZIP_DB_INIT=0`; schema changes must be represented by migrations and recorded in `schema_migrations`.
- Use idempotent migrations where practical (`if not exists`, guarded `do $$` blocks), and keep data/cache backfills separate from schema migrations unless the schema change is unusable without a small deterministic data update.
- If a migration needs PostgreSQL commands that cannot run inside a transaction, such as `create index concurrently`, put `-- migrate: no-transaction` at the top of that SQL file.
- When a DB-backed feature has admin/status/performance diagnostics, update those checks so missing migrations, stale cache tables, and incompatible schema states are visible before or during deployment.
