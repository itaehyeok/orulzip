...
const REQUIRED_PERIOD_MONTHS = DEFAULT_PRICE_BAND_PERIOD_MONTHS;
const REQUIRED_MAP_LEVELS = ["sido", "sigungu", "dong", "apartment"];
const STALE_CACHE_HOURS = 36;
...
async function saveDataHealthRun(run) {
  const result = await query(`
...
function buildCheck({ key, category, title, status, message, metrics = {}, details = [] }) {
  return {
    key,
    category,
    title,
...
function isStale(value) {
  ...
  return Date.now() - timestamp > STALE_CACHE_HOURS * 60 * 60 * 1000;
}
...
function formatMonthKorean(value) {
